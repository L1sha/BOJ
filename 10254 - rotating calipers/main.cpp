...
/*
At first, I tried to solve this prob using distance & binary search
I thought that distance should be monotonic increase, than monotonic decrease
(In range (base + 1) ~ (base + sz - 1), for base : any point of convex hull)
But it wasn't. There are counterexamples.

-> new Algorithm : Rotating calipers
*/
...
void solve() {
    ...
    std::cout << convexHull[maxIdx1].first << ' ' << convexHull[maxIdx1].second << ' ';
    std::cout << convexHull[maxIdx2].first << ' ' << convexHull[maxIdx2].second << '\n';
}
