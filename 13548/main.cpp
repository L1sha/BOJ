#include "bits/stdc++.h"


constexpr int N_MAX = 100'001, A_MAX = 100'001, M_MAX = 100'000;

int N, sqrtN, M;

struct Query {
    
    int s, e, idx;
    bool operator< (const Query& rhs) {
        
        if (s / sqrtN == rhs.s / sqrtN) return e < rhs.e;
        return s / sqrtN < rhs.s / sqrtN;
    }
};

std::array<int, N_MAX> A;
std::array<int, A_MAX> cnt;
std::array<Query, M_MAX> queries;
std::array<int, M_MAX> res;

void get_input() {
    
    std::cin >> N;
    
    for (int idx = 1; idx <= N; ++idx) std::cin >> A[idx];
    
    std::cin >> M;
    
    for (int idx = 0; idx < M; ++idx) {
        
        std::cin >> queries[idx].s >> queries[idx].e;
        queries[idx].idx = idx;
    }
}

void solve() {
    
    sqrtN = sqrt(N);
    
    std::sort(queries.begin(), queries.begin() + M);
    
    std::unordered_set<int> set;
    
    for (int idx = 0, s, e, ans, realAns, prev = -1, l, r; idx < M; ++idx) {
        
        if (queries[idx].s / sqrtN != prev) {
            
            std::fill(cnt.begin(), cnt.end(), 0);
            set.clear();
            
            s = queries[idx].s;
            e = queries[idx].e;
            ans = 0;
            prev = s / sqrtN;
            l = std::max(prev * sqrtN, 1);
            r = std::min((prev + 1) * sqrtN - 1, N);
            
            for (int idx = l; idx <= r; ++idx) set.insert(A[idx]);
            
            for (int idx = s; idx <= e; ++idx) {
                
                ++cnt[A[idx]];
                if (!set.count(A[idx])) ans = std::max(ans, cnt[A[idx]]);
            }
        }
        else {
            
            while (s < queries[idx].s) --cnt[A[s++]];
            
            while (s > queries[idx].s) ++cnt[A[--s]];
            
            while (e < queries[idx].e) {
                
                ++cnt[A[++e]];
                if (!set.count(A[e])) ans = std::max(ans, cnt[A[e]]);
            }
        }
        
        realAns = ans;
        for (int idx = l; idx <= r; ++idx) realAns = std::max(realAns, cnt[A[idx]]);
        
        res[queries[idx].idx] = realAns;
    }
    
    for (int idx = 0; idx < M; ++idx) std::cout << res[idx] << '\n';
}

int main(int argc, char** argv) {
    
    std::ios_base::sync_with_stdio(false);
    std::cin.tie(nullptr);
    std::cout.tie(nullptr);
    
    get_input();
    
    solve();
    
    return 0;
}

