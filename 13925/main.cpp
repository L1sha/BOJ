...
std::array<long long, N_MAX * 4> segTree, lazy1, lazy2, lazy3;
...
void lazyUpdate(const int& cursor, const int& l, const int& r) {
    ...
    if (lazy3[cursor]) {
        ...
        segTree[cursor] = (((lazy3[cursor] * lazy2[cursor] + lazy1[cursor]) % PRIME) * (r - l + 1)) % PRIME;
        
        if (l != r) {
            
            lazy1[cursor * 2] = lazy1[cursor];
            lazy2[cursor * 2] = lazy2[cursor];
            lazy3[cursor * 2] = lazy3[cursor];
            
            lazy1[cursor * 2 + 1] = lazy1[cursor];
            lazy2[cursor * 2 + 1] = lazy2[cursor];
            lazy3[cursor * 2 + 1] = lazy3[cursor];
        }
...
void segUpdate1(const int& ql, const int& qr, const long long& val, int cursor = 1, int l = 1, int r = N) {
    ...
    if (l > qr || r < ql) return;
        ...
        segTree[cursor] = (segTree[cursor] + val * (r - l + 1)) % PRIME;
        
        if (l != r) {
            
            lazy1[cursor * 2] = (lazy1[cursor * 2] + val) % PRIME;
            
            lazy1[cursor * 2 + 1] = (lazy1[cursor * 2 + 1] + val) % PRIME;
        }
    }
    else {
...
void segUpdate2(const int& ql, const int& qr, const long long& val, int cursor = 1, int l = 1, int r = N) {
    ...
    if (l > qr || r < ql) return;
        ...
        segTree[cursor] = (segTree[cursor] * val) % PRIME;
        
        if (l != r) {
            
            lazy1[cursor * 2] = (lazy1[cursor * 2] * val) % PRIME;
            lazy2[cursor * 2] = (lazy2[cursor * 2] * val) % PRIME;
            
            lazy1[cursor * 2 + 1] = (lazy1[cursor * 2 + 1] * val) % PRIME;
            lazy2[cursor * 2 + 1] = (lazy2[cursor * 2 + 1] * val) % PRIME;
        }
    }
    else {
        
        segUpdate2(ql, qr, val, cursor * 2, l, l + (r - l) / 2);
        segUpdate2(ql, qr, val, cursor * 2 + 1, l + (r - l) / 2 + 1, r);
        
        segTree[cursor] = (segTree[cursor * 2] + segTree[cursor * 2 + 1]) % PRIME;
    }
}

void segUpdate3(const int& ql, const int& qr, const long long& val, int cursor = 1, int l = 1, int r = N) {
    ...
    if (l > qr || r < ql) return;
        ...
        segTree[cursor] = (val * (r - l + 1)) % PRIME;
        ...
        if (l != r) {
            ...
            lazy1[cursor * 2 + 1] = 0;
            lazy2[cursor * 2 + 1] = 1;
            lazy3[cursor * 2 + 1] = val;
        }
    }
    else {
        
        segUpdate3(ql, qr, val, cursor * 2, l, l + (r - l) / 2);
        segUpdate3(ql, qr, val, cursor * 2 + 1, l + (r - l) / 2 + 1, r);
